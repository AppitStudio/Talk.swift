...
Pairing authenticates possession of a secret; app names and bundle IDs are not verified publisher identity. Providers must obtain explicit scoped consent. Debuggable hosts, compromised processes, shared credentials, and code authorized for the same Keychain group are outside app-isolation claims. Do not widen entitlements or approve another app's credential request to make tests pass.
