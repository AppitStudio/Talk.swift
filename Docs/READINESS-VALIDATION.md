...
3. Run on native Intel, minimum macOS 12.4 and other target OS versions. Test arbitrary installation paths, multi-user isolation, firewall/filter conditions, sleep/wake, IPv6 policy and resource pressure on a separate test machine. Rosetta and passive idle do not cover those conditions.
