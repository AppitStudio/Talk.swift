...
For the npm skill installer, run `npm run pack:release --prefix Npm` with Node.js 20+. It copies the canonical `Skills/talk-integrations` resources into an isolated package, runs installer tests, checks the exact packed file list, and prints the tarball path under ignored `LocalBuild/NpmPublication`. Review and publish that tarball with `npm publish <tarball-path> --access public`; do not publish the `Npm` source directory directly. Bump `Npm/package.json` for later releases. The skill package version is independent from the Swift SDK version. Both are covered by the repository's [MIT License](LICENSE); retain the package's bundled license when distributing it separately.
