...
- Install an ExtraDock build exposing `com.appit.extradock.docks` version `1.0.0` and create test docks in its own UI. Dock commands need ExtraDock's normal licensed startup; consumers never receive its license credentials. ExtraDock 5 lists every dock, including disabled ones (`isEnabled == false`, `isVisible == false`); a visibility change aimed at a disabled dock answers `disabled`. Builds before 5.0.9 omitted disabled docks entirely.
...
| `isEnabled` | `false` for an ExtraDock 5 dock the user has turned off (it is listed so consumers can manage it, but every visibility change answers `disabled`). Always `true` from ExtraDock 4, which has no disabled state. ExtraDock 5 builds before 5.0.9 omitted disabled docks instead of listing them. |
...
Give ExtraDock one card in your app's Talk settings (DockFlow keeps a dedicated Talk Integrations tab beside its original Integrations pane), driven by a single derived state: not installed, not connected, opening ExtraDock, waiting for Start Pairing, comparing the code, connected (with dock count and whether the grant allows show/hide), disconnected with a saved grant, and error. One Connect… button runs the whole flow; show a generation choice only when both ExtraDock 4 and 5 are installed. A grant that lacks the control scope must be called out on the card ("Connected, but presets can't change your docks") rather than left to a status line, because every mutation then fails. This card lists an app whose API your app actually implements; arbitrary discovery must not fabricate a working feature.

In ExtraDock, **App Connections** (Settings → Integrations) lists generic independent grants regardless of the consumer's name, with the user's label, permissions ("See your docks and whether they're showing", "Show and hide your docks", "Get updates when docks change" — use the same wording in your app), Allowed / Needs attention state, Permissions… and Remove…. Labels remain identified as user-entered/unverified. Neither generation needs a new app release or a consumer-specific panel for each incoming app.
